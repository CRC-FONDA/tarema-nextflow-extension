...
import nextflow.ResourceScaleGradient
...
class TaskRun implements Cloneable {
    ...
    @PackageScope void resolve(BodyDef body) {
        ...
        //def action = ResourceScaleGradient.getAction(this.getName(), this.getContainer())
       //  log.info("We apply the following action: "+ action)
      //  this.getConfig().setProperty("cpus", 2)
      //  this.getConfig().setProperty("memory", 42324243)


        log.info("In resolve method") // Hier sollte klappen
